...
import { getCFPBracket, generateAndSaveCFPBracket, getCFPPicks } from '@/lib/data/cfp'
...
export default async function CFPBracketPage() {
  ...
  let bracket = await getCFPBracket(session.id)
  if (!bracket) {
    try {
      bracket = await generateAndSaveCFPBracket(session.id)
    } catch {
      return (
        <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          <div className="text-center py-24">
            ...
            <p className="text-zinc-500 mb-6 max-w-sm mx-auto">
              Predict at least a few team schedules to generate your CFP bracket.
            </p>
            <Link
              href="/cfb/full-season"
              className="inline-block bg-[#84cc16] text-black font-bold px-6 py-3 rounded-xl hover:bg-[#65a30d] transition-colors"
            >
      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-zinc-500 mt-1">
            Generated from your season predictions.
            {bracket.is_customized && <span className="text-[#65a30d] font-medium"> Customized.</span>}
          </p>
        </div>
      </div>

      <CFPBracketClient bracket={bracket} initialPicks={picks} sessionId={session.id} />
    </div>
  )
}
